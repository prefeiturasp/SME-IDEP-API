...
`docker-compose run --rm djangoapp /bin/bash -c 'cd indice_idep; ./manage.py makemigrations'`.

## Migrate database
`docker-compose run --rm djangoapp /bin/bash -c 'cd indice_idep; ./manage.py migrate'`.
